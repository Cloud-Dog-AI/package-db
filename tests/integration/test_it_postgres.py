...
def test_it_postgres_schema_init_and_rollback(tmp_path: Path, db_settings: DatabaseSettings) -> None:
    if db_settings.dialect.value != "postgresql":
        pytest.skip("postgres integration only in postgres env")
    ...
    with Session(engine) as session:
        ...
        item = repo.create({"name": "it-pg", "status": "new"})
        session.commit()
        assert repo.get(item.id).name == "it-pg"
