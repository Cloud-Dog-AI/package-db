...
class OpenSearchConnector:
    """OpenSearch adapter implementing the connector contract."""

    def __init__(self, *, uri: str, timeout_seconds: int = 30) -> None:
        self._uri = uri
        self._timeout_seconds = timeout_seconds
        self._client = OpenSearch(hosts=[self._build_host(uri)], timeout=timeout_seconds)
        self._cluster_name: str | None = None

    @staticmethod
    def _build_host(uri: str) -> dict[str, Any]:
        parsed = urlparse(uri)
        if not parsed.scheme or not parsed.hostname:
            raise ValueError("OpenSearch URI must include scheme and host")
        host: dict[str, Any] = {
    ...
    @property
    def uri(self) -> str:
        """Return the configured OpenSearch URI."""
        return self._uri

    def capability_report(self) -> dict[str, Any]:
        """Return the OpenSearch adapter capability set."""
        return {
    ...
    def validate_profile(self) -> dict[str, Any]:
        """Validate connectivity to the configured OpenSearch deployment."""
        if not self._client.ping():
            raise RuntimeError("OpenSearch cluster did not respond to ping")
        return {
            "ok": True,
            "source_type": "opensearch",
    ...
    def list_namespaces(self) -> list[dict[str, Any]]:
        """List the single OpenSearch cluster namespace."""
    ...
    def describe_entity(self, namespace: str, entity: str) -> dict[str, Any]:
        """Describe index or alias metadata using OpenSearch mappings and counts."""
        self._ensure_namespace(namespace)
    ...
    def describe_fields(self, namespace: str, entity: str) -> dict[str, Any]:
        ...
        for name, definition in sorted(properties.items()):
            field = {"name": name, "types": sorted(self._extract_types(definition))}
    ...
    def read(
        ...
        """Read documents from an index or alias using OpenSearch query DSL."""
        self._ensure_namespace(namespace)
        body: dict[str, Any] = {"query": filter or {"match_all": {}}}
        body["_source"] = self._projection_to_source(projection)
        if sort:
            body["sort"] = self._sort_to_os(sort)
        response = self._client.search(index=entity, body=body, size=int(limit or 50))
        return [self._normalise_hit(hit) for hit in response.get("hits", {}).get("hits", [])]

    def create(self, namespace: str, entity: str, document: dict[str, Any]) -> dict[str, Any]:
        """Index a document into an OpenSearch entity."""
        self._ensure_namespace(namespace)
        payload = dict(document or {})
        doc_id = str(payload.pop("_id", payload.pop("id", "")) or "")
        kwargs: dict[str, Any] = {"index": entity, "body": payload, "refresh": True}
        if doc_id:
            kwargs["id"] = doc_id
    ...
    def update(self, namespace: str, entity: str, filter: dict[str, Any], update: dict[str, Any]) -> dict[str, Any]:
        ...
        response = self._client.update_by_query(
            index=entity,
            body={"query": filter or {"match_all": {}}, "script": script},
            refresh=True,
            conflicts="proceed",
        )
        return {
    ...
    def delete(self, namespace: str, entity: str, filter: dict[str, Any]) -> dict[str, Any]:
        ...
        response = self._client.delete_by_query(
            index=entity,
            body={"query": filter or {"match_all": {}}},
            refresh=True,
            conflicts="proceed",
        )
    ...
    def count(self, namespace: str, entity: str, filter: dict[str, Any] | None = None) -> int:
        ...
        self._ensure_namespace(namespace)
        response = self._client.count(index=entity, body={"query": filter or {"match_all": {}}})
    ...
    def sample_shapes(self, namespace: str, entity: str, n: int = 10) -> list[dict[str, Any]]:
        ...
        self._ensure_namespace(namespace)
        response = self._client.search(index=entity, body={"query": {"match_all": {}}}, size=int(max(1, n)))
        return [self._normalise_hit(hit) for hit in response.get("hits", {}).get("hits", [])]
    ...
    def schema_change_apply(self, plan: dict[str, Any]) -> dict[str, Any]:
        ...
        if op_type == "drop_index":
            name = str(parameters.get("name") or self._template_name(entity, parameters)).strip()
    ...
    def _namespace(self) -> str:
        if self._cluster_name is None:
            self._cluster_name = str(self._client.info().get("cluster_name", "opensearch"))
        return self._cluster_name

    def _ensure_namespace(self, namespace: str) -> None:
        if namespace and namespace != self._namespace():
            raise ValueError(f"Unknown OpenSearch namespace: {namespace}")
    ...
    def _entity_type(self, entity: str) -> str:
        ...
        if entity in self._list_indices():
            return "index"
        raise ValueError(f"Unknown OpenSearch entity: {entity}")
    ...
    @staticmethod
    def _projection_to_source(projection: dict[str, Any] | None) -> bool | dict[str, Any]:
        if not projection:
            return True
    ...
    @staticmethod
    def _sort_to_os(sort: list[dict[str, Any]]) -> list[dict[str, Any]]:
        return [
            {
                str(item.get("field", "")): {
                    "order": "asc" if str(item.get("direction", "asc")).lower() == "asc" else "desc"
                }
            }
            for item in sort
    ...
    def _build_update_script(update: dict[str, Any]) -> dict[str, Any]:
        ...
        statements = []
        if set_values:
            statements.append("for (entry in params.set_values.entrySet()) { ctx._source[entry.getKey()] = entry.getValue(); }")
        if unset_names:
            statements.append("for (field in params.unset_fields) { ctx._source.remove(field); }")
        if not statements:
            raise ValueError("OpenSearch update requires $set and/or $unset")
        return {
            "lang": "painless",
    ...
    def _matching_templates(self, entity: str) -> list[dict[str, Any]]:
        ...
        for item in response.get("index_templates", []):
            ...
            if not any(fnmatch.fnmatch(entity, pattern) for pattern in patterns):
                continue
            properties = template.get("template", {}).get("mappings", {}).get("properties", {})
            results.append(
                {
                    "name": item.get("name"),
                    "type": "index_template",
    ...
    def _plan_create_entity(self, namespace: str, entity: str, parameters: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "after_state": {
                "entity_exists": True,
                "aliases": sorted((dict(parameters.get("aliases") or {})).keys() | ({str(parameters.get('alias', '')).strip()} - {""})),
            },
        }
    ...
    def _plan_drop_entity(self, namespace: str, entity: str, parameters: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "entity": entity,
            "parameters": parameters,
            "before_state": {"entity_exists": True, "entity_type": entity_type, "indices": self._resolve_targets(entity)},
